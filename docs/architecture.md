...
│  │                      MonitoringAgent                            │   │
│  │  - 생명주기 관리 (start / stop)                                   │   │
│  │  - ScheduledExecutorService (콘솔 출력 / 원격 폴링)               │   │
│  └─────────┬──────────────────────┬──────────────────────────┬────┘   │
│            │                      │                          │         │
│  ┌─────────▼──────┐   ┌───────────▼────────┐   ┌────────────▼──────┐ │
│  │JvmMetricsCollector│ │  TransactionTracer  │   │RemoteJvmCollector │ │
│  │ (로컬 JVM MXBean) │ │  + SpanStorage      │   │ (JMX RMI 원격)    │ │
│  │                 │  │                     │   │                   │ │
│  │ · Heap/NonHeap  │  │ · @Trace 자동 추적    │   │ · 5s 폴링         │ │
│  │ · GC 상세       │  │ · 수동 Span API      │   │ · 자동 재연결      │ │
│  │ · Thread 상태   │  │ · 링 버퍼(1000건)    │   │ · snapshot 캐시   │ │
│  │ · OS 물리 메모리 │  │                     │   │ · threadList 캐시 │ │
│  │ · Buffer Pool   │  └───────────┬─────────┘   └────────────┬──────┘ │
│  │ · JIT 컴파일    │              │                           │         │
│  └─────────┬───────┘              │                           │         │
│            │                      │                           │         │
│  ┌─────────▼──────────────────────▼───────────────────────────▼──────┐ │
│  │                       PrometheusExporter                            │ │
│  │  로컬 JVM 메트릭 → Prometheus text/plain 0.0.4 형식 변환             │ │
│  └─────────────────────────────────┬───────────────────────────────────┘ │
│                                    │                                      │
│  ┌─────────────────────────────────▼───────────────────────────────────┐ │
│  │                       MetricsHttpServer                              │ │
...
│  │                                                                      │ │
│  │  GET /dashboard     HTML 대시보드 (Chart.js, classpath 로드)           │ │
│  │  GET /jvm           로컬 JVM 메트릭 JSON                               │ │
│  │  GET /metrics       Prometheus 형식 텍스트                             │ │
│  │  GET /traces        최근 스팬 JSON (최대 50건)                          │ │
│  │  GET /api/stats     스팬 통계 JSON (totalSpans / errorRate / avgMs)   │ │
│  │  GET /health        헬스 체크 JSON                                     │ │
│  │  GET /remote/jvm     원격 JVM 메트릭 JSON                              │ │
│  │  GET /remote/status  원격 JMX 연결 상태 JSON                           │ │
...
|--------|------|--------------|------|
| GET | `/dashboard` | `text/html` | 실시간 그래프 대시보드 |
...
| GET | `/remote/threads` | `application/json` | 원격 JVM 스레드 목록 (상태·CPU·잠금·스택) |
...
├── core/
│   ├── JvmMetricsCollector    로컬 JVM MXBean 수집
...
├── server/
│   └── MetricsHttpServer      JDK 내장 HTTP 서버
├── exporter/
...
### Java Monitor 설정 (monitor.properties)
...
```bash
java -jar java-monitor-1.0.0.jar
# 또는 설정 파일 직접 지정
java -jar java-monitor-1.0.0.jar /etc/myconfig/monitor.properties
```
