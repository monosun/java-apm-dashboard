...
대상 JVM에 `-javaagent:java-monitor-1.5.0-agent.jar=port=7979`를 부착한 뒤 이 설정을 활성화합니다.
...
```
java-monitor-1.5.0.jar 더블클릭 ❌  →  start.bat 더블클릭 ✅
```
...
```bash
# 포트 변경
java -Dserver.http.port=18080 -jar java-monitor-1.5.0.jar
...
java -Xms512m -Xmx2g -XX:+UseG1GC \
     -jar java-monitor-1.5.0.jar /etc/monitor/prod.properties
```
