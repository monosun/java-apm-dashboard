# Java Performance Monitor — 설정 매뉴얼
...
9. [원격 JVM 연결 단계별 설정](#9-원격-jvm-연결-단계별-설정)
10. [트러블슈팅](#10-트러블슈팅)
...
```bash
java -jar java-monitor-1.0.0.jar /etc/monitor/prod.properties
```
...
```
java-monitor-1.0.0.jar 더블클릭 ❌  →  start.bat 더블클릭 ✅
```
...
```bash
# 포트 변경
java -Dserver.http.port=18080 -jar java-monitor-1.0.0.jar
...
java -Xms512m -Xmx2g -XX:+UseG1GC \
     -jar java-monitor-1.0.0.jar /etc/monitor/prod.properties
```
