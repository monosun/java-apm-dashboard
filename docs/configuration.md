...
```bash
java -jar java-monitor-1.4.0.jar /etc/monitor/prod.properties
```
...
대시보드의 **최근 트레이스(Span) 섹션** 표시 여부를 제어합니다. `false`로 설정하면 대시보드에서 트레이스 섹션이 숨겨집니다. Span 수집 자체는 영향을 받지 않습니다.

```properties
# 트레이스 섹션 숨기기
server.traces.enabled=false
```

`/api/config` 엔드포인트를 통해 현재 값을 확인할 수 있습니다.
...
대상 JVM에 `-javaagent:java-monitor-1.4.0-agent.jar=port=7979`를 부착한 뒤 이 설정을 활성화합니다.
...
```
java-monitor-1.4.0.jar 더블클릭 ❌  →  start.bat 더블클릭 ✅
```
...
```bash
# 포트 변경
java -Dserver.http.port=18080 -jar java-monitor-1.4.0.jar
...
java -Xms512m -Xmx2g -XX:+UseG1GC \
     -jar java-monitor-1.4.0.jar /etc/monitor/prod.properties
```
