...
빌드 결과:
- `target/java-monitor-1.12.0.jar` — 메인 APM 대시보드 서버
- `target/java-monitor-1.12.0-agent.jar` — 대상 JVM 부착용 Agent Library (`-javaagent:`)
- `target/java-monitor-1.12.0-integration.jar` — 웹앱 Trace ID 연동 라이브러리 (`WEB-INF/lib`)
...
```bash
java -javaagent:java-monitor-1.12.0-agent.jar=port=7979 -jar your-app.jar
```
...
vm.loadAgent("/path/to/java-monitor-1.12.0-agent.jar", "port=7979");
vm.detach();
```
...
```bash
# 1. JVM에 agent 부착
java -javaagent:java-monitor-1.12.0-agent.jar=port=7070 -jar myapp.jar

# 2. integration JAR를 WEB-INF/lib 또는 클래스패스에 추가
cp java-monitor-1.12.0-integration.jar myapp/WEB-INF/lib/
```
...
```bash
java -Dserver.http.port=8080 -Dagent.enabled=true -Dagent.host=myapp -jar java-monitor-1.12.0.jar
```
