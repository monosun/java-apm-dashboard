...
| **HTTP 요청 처리** | Tomcat RequestProcessor 스레드별 처리 시간·URI·Remote IP — Live Stack 뷰와 자동 연동 |
...
빌드 결과:
- `target/java-monitor-1.6.0.jar` — 메인 APM 대시보드 서버
- `target/java-monitor-1.6.0-agent.jar` — 대상 JVM 부착용 Agent Library
...
```bash
java -javaagent:java-monitor-1.6.0-agent.jar=port=7979 -jar your-app.jar
```
