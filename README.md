...
| **HTTP 요청 처리** | Tomcat RequestProcessor — Method·URI·Remote IP(IPv6 정규화)·스레드명 표시, 행 클릭 시 스레드 상세·스택 트레이스 모달 |
...
빌드 결과:
- `target/java-monitor-1.7.0.jar` — 메인 APM 대시보드 서버
- `target/java-monitor-1.7.0-agent.jar` — 대상 JVM 부착용 Agent Library
...
│       + ⟳ 즉시 갱신 버튼                                    │
│     - Live Stack: HTTP 워커 스레드에 🌐 URI ← RemoteIP 배지  │
│   • 활성 HTTP 요청 테이블 (Tomcat RequestProcessor)          │
│     - Method 배지 / URI / Remote IP(IPv6 정규화) / 스레드명  │
│     - 행 클릭 → 요청 상세 모달                               │
...
```bash
java -javaagent:java-monitor-1.7.0-agent.jar=port=7979 -jar your-app.jar
```
