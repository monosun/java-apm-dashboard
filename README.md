...
빌드 결과:
- `target/java-monitor-1.9.0.jar` — 메인 APM 대시보드 서버
- `target/java-monitor-1.9.0-agent.jar` — 대상 JVM 부착용 Agent Library
...
```bash
java -javaagent:java-monitor-1.9.0-agent.jar=port=7979 -jar your-app.jar
```
...
vm.loadAgent("/path/to/java-monitor-1.9.0-agent.jar", "port=7979");
vm.detach();
```
