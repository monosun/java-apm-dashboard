...
빌드 결과:
- `target/java-monitor-1.12.2.jar` — 메인 APM 대시보드 서버
- `target/java-monitor-1.12.2-agent.jar` — 대상 JVM 부착용 Agent Library (`-javaagent:`)
- `target/java-monitor-1.12.2-integration.jar` — 웹앱 Trace ID 연동 라이브러리 (`WEB-INF/lib`)
...
```bash
java -javaagent:java-monitor-1.12.2-agent.jar=port=7979 -jar your-app.jar
```
...
vm.loadAgent("/path/to/java-monitor-1.12.2-agent.jar", "port=7979");
vm.detach();
```
...
## Trace ID 연동 (v1.10.0+, v1.12.1 유지)
...
```bash
# 1. JVM에 agent 부착
java -javaagent:java-monitor-1.12.2-agent.jar=port=7070 -jar myapp.jar

# 2. integration JAR를 WEB-INF/lib 또는 클래스패스에 추가
cp java-monitor-1.12.2-integration.jar myapp/WEB-INF/lib/
```
...
```bash
java -Dserver.http.port=8080 -Dagent.enabled=true -Dagent.host=myapp -jar java-monitor-1.12.2.jar
```
...
### v1.12.2
- **Tomcat JDBC pool 커넥션 풀 메트릭 수집 수정** — `Catalina:type=DataSource` MBean의 실제 속성명 사용
  - 기존: HikariCP/DBCP2 스타일 속성명(`TotalConnections`, `numBusyConnections` 등) 사용 → 전부 -1 반환
  - 수정: Tomcat JDBC pool `ConnectionPoolMBean` 실제 속성명(`NumActive`, `NumIdle`, `Size`, `WaitCount`, `MaxActive`, `MinIdle`) 사용
  - `queryTomcatJdbcPools()` 전용 메서드 분리
  - 풀 이름에서 JMX 자동 인용 따옴표 제거 (`"jdbc/securityportal"` → `jdbc/securityportal`)
  - 동일 풀이 여러 webapp 컨텍스트에 중복 등록된 경우 첫 번째만 반환
...
- **HikariCP MBean 패턴 확장** — `com.zaxxer.hikari:*` 전체 스캔으로 등록 방식에 무관하게 자동 감지
  - `type=PoolStats,name=<pool>` (Dropwizard metrics 통합) 우선 수집
  - `type=Pool (<pool>)` + `type=PoolConfig (<pool>)` (HikariCP 내장 JMX `registerMbeans=true`) 조합 지원
  - 동일 풀 중복 카운팅 방지
- **대시보드 UX 개선**
  - CPU Top10 기본 갱신 주기 5초 → 60초 (불필요한 폴링 감소)
  - 스레드 상세 모달: 로딩 완료 후 닫기 시 확인 다이얼로그 표시 (실수 닫기 방지)
